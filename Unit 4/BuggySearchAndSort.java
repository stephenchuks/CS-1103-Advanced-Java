/**
 The first problem I discovered was that the the outer and and the
 innerloop of the bubbleSort incremented i;
 
 I also observed that there was a missing wildcard to indicate importation of all classes in the package.
 
  In the selectionsort, the conditional statement in the loop did not reference the right variable. Was probably a typo.
  
   In the insertionsort, since the is initialized to top -1, and initial value of top is 1, the pos should handle
  a case where pos is = 0 by including the by changing the conditional operator to => 0.
  
  In "insertionSort" method , In the while it should be "pos >=0" instead of "pos>0" .
*/
